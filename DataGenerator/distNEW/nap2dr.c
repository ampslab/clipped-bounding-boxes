/* nap2dr: convert arbitrary dimensional ASCII formatted NUMBERED POINTS to
#//
#// Copyright (c) 1994 - 2013 Norbert Beckmann  <nb@informatik.uni-bremen.de>
#//
   lhlh.. DOUBLE RECTANGLE representation (usable for RSTree) */
...
{
  FILE *infile, *outfile;
  int NumbOfDim, itemsread, d;
  Rpint numbrecs, lineNumb;
  boolean endOfFile, badEof;
  double coord;
  ...
  if (argc != 4) {
    ...
    fprintf(stderr,"%s %s\n",argv[0],"converts NUMBERED ASCII POINTS to lhlh.. DOUBLE formatted RECTANGLES.");
    exit(1);
  }
  else {
    ...
    numbrecs= 0; endOfFile= FALSE; badEof= FALSE;
    for (;;) {
      itemsread= fscanf(infile,strans("%P",s),&lineNumb);
      if (itemsread == EOF) {
        break;
      }
      for (d= 0; d < NumbOfDim; d++) {
        itemsread= fscanf(infile,"%lf",&coord);
        if (itemsread == EOF) {
          badEof= TRUE;
          endOfFile= TRUE;
          break;
    ...
    fprintf(stderr,strans(" Last line number: %P\n",s),lineNumb);
  }
  return 0;
}
