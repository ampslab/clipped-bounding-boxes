/* MDapr: see option -help */
#//
#// Copyright (c) 1994 - 2015 Norbert Beckmann  <nb@informatik.uni-bremen.de>
#//
/* MDapr: see option -help *** sort by absolute nearest */
/* MDapr.c: grep Param. */
...
void SetGlobrectOrigin(t_RT rst, typinterval *r, typcoord *p);
...

{
  ...
  typcoord *start= malloc(pointSize);
  t_RT rst;
  t_LRU LRU;
  boolean isRTonDisk;
  t_DQ distQ;
...
"Depending on the first argument -n(earest) / -f(arthest), the distance\n");
    printf(
"between the starting point and the current rectangle is the\n");
    printf(
"MINIMUM / MAXIMUM regarding the remaining (following) rectangles of the\n");
    printf(
"file.\n");
    printf(
"The algorithm starts at the origin of the distribution.\n");
    exit(1);
  }
...
"       ",argv[0]," ( -n | -f ) inputfile outputfile\n");
    ...
    SetGlobrectOrigin(rst,globrect,start);
    PrintGlobrectStart(globrect,start);
    outfile= fopen(argv[3],"wb");
    InitDistQueryIdent(&distQ);
    printf("Distance Query: WORKING ...\n");
    ...
    if (! NewDistQuery(rst,start,CalcEucl,sort,minDist,0.0,NULL,0,NULL,AlwaysTrue,AlwaysTrue,2048,FALSE,&distQ)) {
...

{
  ...
  printf(strans("%s%L\n",s),"#records in R-Tree: ",numbofrecords);
}
