...
#define RANDSEED 7901 /* <--- set Param. */
...
int maxstep;
Rpint AvgPerClust, rectCount, writeEach;
boolean randAxes;
...

{
  ...
  if (argc != 3 || strcmp(argv[1],"-d") != 0 && strcmp(argv[1],"-q") != 0) {
    ...
    /* CDpha first produces partitions by recursively splitting the world into
       -----------------------------------------------------------------------
       two parts, modulo alternating or randomly choosing the split axis
       -----------------------------------------------------------------
       before each split. Around one of the corners of each partition,
       ---------------------------------------------------------------
       clusters of about AvgPerClust rectangles are produced.
       ------------------------------------------------------
       Distribution: clusters around the corners of partitions
       Order: related to Z-order
       start[d]: origin-corner of the world
       -----------------------------------------------------------------------
       
       The partition-world "pw", containing the cluster centers, is a square
       ...
       CellWidth[d] = PartWidth[d] / pow(AvgPerClust, 1/NumbOfDim) and
       PartWidth[d] is the width of the underlying partition along the
       concerning dimension d.
       Cells are created as follows:
       One of the pow(2, NumbOfDim) corners of each partition is randomly
       selected. On the average AvgPerClust cells are randomly distributed
       with their centers around it.
       Let distance denote the distance between the chosen partition-corner
       and the cell-center now.
       The maximum distance along each dimension d is defined as PartWidth[d],
       the maximum total sqared distance is defined as
       PartWidth[d0]^2 + PartWidth[d1]^2 + .. + PartWidth[dn]^2,
       di = 0 .. NumbOfDim - 1.
    ...
    */
    ...
    WorldWidth= 1.0;
    /* ---------- CDpha --- distribution Param. setting ---------- */
    /*
      ...
      - randAxes: if set to TRUE, the split axis is chosen randomly otherwise
                  it alternates.
      - AvgPerClust: average number of rectangles per cluster. The actual
                     number of rectangles varies between 1 and
                     2 * AvgPerClust - 1.
...

{
  int d;
  Rpint i, pc;
  typatom start[NumbOfDim], vector[NumbOfDim];
  typcerect rect;
  double CellWidth[NumbOfDim], PartWidth[NumbOfDim], RealRand, SqrRad;
  ...
  ***** end:   also produce what CDpar would produce *****/
  ...
  for (d= 0; d < NumbOfDim; d++) {
    ...
    CellWidth[d]= PartWidth[d] / pow((double)AvgPerClust, 1.0 / NumbOfDim);
    rect.e[d]= CellWidth[d] * 0.5 * wi[d];
  }
  
  RealRand= drand48();
  pc= (2 * AvgPerClust - 1) * RealRand + 1;
  for (i= 0; i < pc; i++) {
    do {
      SqrRad= 0.0;
      for (d= 0; d < NumbOfDim; d++) {
        RealRand= drand48();
        vector[d]= RealRand;
        SqrRad+= vector[d] * vector[d];
      }
    } while (SqrRad > 1.0);
    
    for (d= 0; d < NumbOfDim; d++) {
      if (mrand48() >= 0) {
        rect.c[d]= start[d] + vector[d] * PartWidth[d];
      }
      else {
        rect.c[d]= start[d] - vector[d] * PartWidth[d];
      }
