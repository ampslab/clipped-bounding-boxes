/* vidi.c visualize a rectangle distribution */
#//
...
typedef typinterval  typrect[2];
/* input format of coordinates,
   conversion to Xwindow format see "typrect" below */
/*------------------------------------------------------------*/
...

{
  ...
  if (argc > 1) {
    ...
    fprintf(stderr,"%s\n",
"vidi displays rectangle data files"
    );
    fprintf(stderr,"%s",
"data format:  Xlow, Xhigh, Ylow, Yhigh: "
    );
...
"Usage:"
    ...
    fprintf(stderr,"%s\n",
"  vidi filename [ xl xh yl yh ] [ -wwidth ] [ -m ] [ -b ]"
    );
    fprintf(stderr,"%s\n",
...
"Example:"
    ...
    fprintf(stderr,"%s\n",
"vidi rea02 -120.4 -118.1 36.1 38.4 -m -w960 -b"
    );
    fprintf(stderr,"%s\n",
...

{
  ...
  if (data->ok) {
    ...
    /* from vidi.c: numbRects= GetLOF(argv[1]) / (4 * sizeof(typcoord)); */
    /* Computing the number of records in a text file may be extremely time
       consuming and would delay the usability of the program. So we abstain
       from it for now (and set it to -1 instead). */
    numbRects= -1;
...

{
  ...
  if (data != NULL) {
    fprintf(stderr,"vidi: quit:%s\n",data->message);
  }
  else {
    fprintf(stderr,"vidi: quit\n");
  }
  exit(0);
}
...

{
  ...
  /* from vidi.c: if (fread(datarect,sizeof(datarect),1,data->f)) { */
  if (RdTextToRect(data->f,datarect)) {
    rectCount++;
    
    // Begin data to X conversion
    lux= (int)((datarect[0].l - data->WorldXL) * data->HorScale + 0.5);
    if (data->MathOri) {
      luy= (int)((data->WorldYlength - (datarect[1].h - data->WorldYL)) * data->VerScale + 0.5);
    }
    else {
      luy= (int)((datarect[1].l - data->WorldYL) * data->VerScale + 0.5);
    }
    width= (int)((datarect[0].h - datarect[0].l) * data->HorScale + 0.5);
    height= (int)((datarect[1].h - datarect[1].l) * data->VerScale + 0.5);
