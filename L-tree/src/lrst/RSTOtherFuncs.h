...
boolean  RegionCountNoDead(t_RT         rst,
         ...
         /* RegionCountNoDead performs a L-ized RegionQuery on the tree
            referenced by rst. It does not return records but only counts the
...
boolean LTreeComp( t_RT r, FILE *stream, const Rint num_dead, const Rint method );

         /* Recursively traverses the tree and computes deadly points for
            node each. */
...
boolean  PathsDump(t_RT  rst);
         ...
         /* This implementation normally maintains a single path of page
            ...
            Additionally, for the different levels of the tree, several data
...
boolean  CustomPathsDump(t_RT  rst, boolean useful_access);
