...
typedef struct {
               Rshort      nofentries;
               Rshort      makeSize4;		/* filler */
               Rshort      makeSize6;		/* alignment filler, size > 4 */
               typatomkey  coerceAlignment[];	/* strictly unused! */
               } typnodeinfo, *refnodeinfo;	/* aligned for typatomkey (!) */
...
typedef struct {
               ...
               /*** NEW (L-Tree) STUFF ***/
               Rint         max_dpoints; // maximum #deadly points per node
               Rint         method;
               /*** END OF NEW (L-Tree) STUFF ***/
...
typedef struct { 
               ...
               Rlint     DQ_PQelems;
               Rlint     useful_leaf_accesses; /* NEW (L-Tree) STUFF */
               } typcount, *refcount;
...
typedef struct {
                Ruint       entLen;
                void*       dpoints;
} DPointInfo; // meta data for deadly points
/* ----------- END OF NEW (L-Tree) STUFF: */
...
typedef struct {
               ...
               ValueArray     CSCM_Enls;
               refnode        S_NSibl;
               refnode        S_NSplit;
               typinterval    *S_allrect;
               ValueArray     DDPS_Areas;
               typinterval    *DDPN_rectN;
               typinterval    *DDPN_rectNSib;
               uWordsBytes    *DDPN_gone;
               typinterval    *DOR_effDelRect;
               IndexArray     SADDD_I;
               ...
               typcoord       *EDDE_allcenter;
               typcoord       *EDDE_newrectcenter;
               typcoord       *EDDE_center;
               ValueArray     EDDE_distarr;
               IndexArray     EDDE_I;
               uWordsBytes    *EDDE_chosen;
               typinterval    *APAMD_helprect;
               /* ----------- in RSTree: */
               void           *IR_entry;
               /* ----------- in RSTQuery: */
