...
static Rint HeightPredict(Rfloat availPages, Rfloat fanout);
...
{
  /* Early detection of memory limitation (directory): The insertion of a data
     record may trigger a split of the root. ==> Height + 1 pages should be
     reserved (for algorithms without re-insertion).
     The unknown future height of the tree is predicted, based on an assumed
     minimum space utilization and the number of pages currently allocated.
     Assumed Space Utilisation in %: */
# define ASS_SPC_UTL 40
  /* Should be set to the lowest value, where a data insertion may
     (nevertheless) trigger a root split. */
  Rint assEntPerPage;
  ...
  assEntPerPage= ASS_SPC_UTL * (*par).DIR.M / 100;
  if (assEntPerPage < DIR_MIN_ENTRY_QTY) {
    assEntPerPage= DIR_MIN_ENTRY_QTY;
  }
  maxdirsplit= ((*ver).DIR.reinsertqty + 1) * HeightPredict(nofdirpages,assEntPerPage) + 1;
  ...
  /* Re-insertion: worst case if OverflowTreatment is propagated once per level */
...
/************************************************************************/
/* nNodes must be Rfloat to avoid overflow for availPages near MAX(Rpnint) */
...
static Rint HeightPredict(Rfloat availPages, Rfloat fanout) {
  ...
  while (nNodes < availPages) {
    nNodes+= pow(fanout,height);
    height++;
  }
  return height;
}
...

{
  ...
  /* reading: */
  (*c).DIR.demand= 0; (*c).DATA.demand= 0, (*c).useful_leaf_accesses= 0;
