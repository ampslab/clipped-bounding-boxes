...
static void PrintEntry(RSTREE R,
                       ...
                       Rint level,
                       FILE *stream);
static void PrintSplitPoint(RSTREE R,
                            typcoord *p,
                            FILE *stream);
...

{
  ...
  if (level != 0) {
    ...
    fprintf(stream,strans("b: %5I\n",s),(Rint)(*n).s.splitnoe);
    /* why "b"?: n, s already used; mnemonic: began with b entries */
    PrintSplitPoint(R,(*n).s.splitmid,stream);
    ...
    // -- DS --
    // We will go through all entries again, collect/compute points that will be
    // considered for skyline computation.them, and store them in a
    // point-by-point format (rather than coordinate-by-coordinate).
    // We assume only 2D case, i.e., 4 sets of candidates for each corner:
/*    typcoord *ne = allocM( sizeof(typcoord) * ((*n).s.nofentries) * (*R).numbofdim );
    typcoord *se = allocM( sizeof(typcoord) * ((*n).s.nofentries) * (*R).numbofdim );
    typcoord *sw = allocM( sizeof(typcoord) * ((*n).s.nofentries) * (*R).numbofdim );
    typcoord *nw = allocM( sizeof(typcoord) * ((*n).s.nofentries) * (*R).numbofdim );

    // volume of current corner points:
    Rfloat ne_vol = 0.0, se_vol = 0.0, sw_vol = 0.0, nw_vol = 0.0;
    ...
    for (i= 0; i < (*n).s.nofentries; i++) {
      ...
      for (d= 0; d < (*R).numbofdim; d++) {
        const Rfloat lo = (*(refinterval)ptrNi).l;
        const Rfloat hi = (*(refinterval)ptrNi).h;

        // NorthEast points:
        ne[i*(*R).numbofdim + d] = hi;
        // SouthEast point:
        se[i*(*R).numbofdim + d] = d == 0 ? hi : lo;
        // SoutWest point:
        sw[i*(*R).numbofdim + d] = lo;
        // NorthWest point:
        nw[i*(*R).numbofdim + d] = d == 0 ? lo : hi;

        ptrNi+= SIZEinterval;
      }

      // FIXME: looks like ptrNi is not correctly advanced
    	ptrNi+= (*R).DIR.entLen;
    }
    ...
    for (i= 0; i < (*n).s.nofentries; i++) {
      ...
      fprintf(stream,"NE: ");
      for (d= 0; d < (*R).numbofdim; d++) {
        fprintf(stream,"% 23.16e ", ne[i*(*R).numbofdim + d]);
      }
      fprintf(stream,"\n");
    }*/
...
void SetOffsets(RSTREE R,
  ...
  /* offsets in nodes: */
  splitmidOffset= sizeof(typnodeinfo);
  
  /* Does the compiler behave as expected?: */
  if ((splitmidOffset & SIZEcoord - 1) != 0 || splitmidOffset > SIZEcoord) {
    fprintf(stderr,
    "RST: SetOffsets: WARNING: UNEXPECTED ALIGNMENT:\n");
    fprintf(stderr,
    strans("sizeof(typnodeinfo) = %I, should be %I\n",s),splitmidOffset,SIZEcoord);
  }
  
  entOffset= splitmidOffset + numbOfDim * SIZEcoord;
