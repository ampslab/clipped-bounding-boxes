...
LRUentry *LRUEntry(t_LRU lru,
  ...
  *pageAdr= NULL;
  ...
  if (avail) {
    ...
    lof= LenOfF(file);
    if (lof < 0) {
      /* failure accessing the file (status) */
      return NULL;
    }
    curr= NewEntry(LRU,index,filePage);
    if (curr == NULL) {
    ...
    /* curr in []-lists now */
    if (lof < (pageNr + 1) * LRUPageSize(lru)) {
#ifndef COUNTS_OFF
      if ((*LRU).count.on) {
        (*LRU).count.new++;
      }
    ...
    /* new || fetched entry establishment: */
    (*curr).pub.locked= 0;
    (*curr).pub.write= FALSE;
    AddHeadEntry(LRU,curr);
    /* curr in head-tail-list too now */
    ...
    if ((*LRU).qty > (*LRU).max) {
      ...
      if (! CutTailEntry(LRU,&tail)) {
        //Possible errors:
        //displacement_Locks, displacement_WriteFailed.
        //Besides the error/warning messages by WrPage()/CutTailEntry(), the
        //issue should also directly fed back to the caller. So, missing
        //to have other feedback parameters here, dispose the new entry and
        //return NULL.
        ReleaseEntry(LRU,curr);
        DisposeEntry(LRU,index,filePage);
        return NULL;
...

{
  *tail= (*LRU).tail;
  ...
  if (*tail == NULL || *tail == (*LRU).head) {
    ...
    setLRUwarn(LRU,"LRUBuf.CutTailEntry",displacement_Locks);
    return FALSE;
  }
  if ((**tail).pub.write) {
    if (! WrPage((**tail).fp.f,(*LRU).psize,(**tail).fp.p,(**tail).pub.adr)) {
      setLRUwarn(LRU,"LRUBuf.CutTailEntry",displacement_WriteFailed);
      return FALSE;
    }
#ifndef COUNTS_OFF
    if ((*LRU).count.on) {
