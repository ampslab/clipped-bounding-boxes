...
/** Definitions:
    ...
    A simplified and less powerful function is also provided. Besides
    returning a reference to the requested buffer entry, it directly  provides
    access to the corresponding buffer page.
...
/* The parameter "Done"
   ...
   Displacement Errors
   ===================
   The following mainly concernes buffer handling using locks, but NOTE the
   additional explanations under "Concerns both functions" below.
   ...
   *) Applies to the fully equipped buffer accessing functions HashLRUEntry()
      and NewLRUEntry(), does not apply to the simplified buffer accessing
      function LRUEntry().
...
/* Concerns both functions, HashLRUEntry() and NewLRUEntry():
   ...
   - If a requested page is not accessible (be it an existing or a new one),
     ...
     HashLRUEntry() cannot access the indicated file.
     NewLRUEntry() cannot allocate enough memory for a new buffer entry.
     
     The sole non fatal error occurs when displacement fails. But though being
     non fatal (the new entry will become an accessible member of the LRU
     buffer), there are two different levels of severity:
     1. No entry could be displaced, because no lock-free entry was found.
     This is reported by the implementation in that (besides a warning emitted
     on stderr) an immediate call of LRUError() returns the error
     "displacement_Locks". For this case the caller can easily provide a
     handling. For further information see: LRUError(), GetLRUCapUse(),
     ModifLRUCap().
     2. The entry to be displaced has the write flag set, but could not be
     displaced, because it could not be written to its file. This is, though
     the new entry will still become an accessible member of the LRU buffer,
     obviously a severe error. In this case an immediate call of LRUError()
     returns the error "displacement_WriteFailed".
     
     If Done == FALSE and refEntry != NULL, a displacement error has
     occurred. LRUError() will return the specific error.     
...
LRUentry *LRUEntry(t_LRU   LRU,
                   File    f,
                   Rpnint  pageNr,
                   void*   *pageAdr);

/* LRUEntry() provides simplified access to the address of a buffered page of
   file f. It does not provide detailed feedback.
   
   If successful, the function returns the address of the buffer entry,
   identified by f and pageNr, and provides the address of the corresponding
   buffer page in *pageAdr. Pages in front of EOF are attempted to be read
   from secondary memory. Pages behind EOF are newly created in the buffer.
   If the function fails, it returns NULL, *pageAdr is set to NULL, and a
   message is printed to stderr.
   NOTE that the occurrence of displacement errors prevents the new entry from
   being integrated into the buffer, as for all other errors. This is a
   difference with regard to HashLRUEntry() and NewLRUEntry(), where
   displacement errors do not prevent a new entry from being integrated.
   
   NOTE that after writing to a buffer page, the write flag of the
   corresponding buffer entry should be set to TRUE. See also SetLRUWrFlag().
   
   LRU:       The reference of an LRU buffer previously created by the
              function NewLRU().
   f:         A file, already being buffered or newly to be buffered.
   pageNr:    The page number of the page to be read or (re-)written.
   pageAdr:   The address of a void pointer, the address of the buffer page is
              assigned to.
      
   The function expects:
   - an LRU buffer, previously created by NewLRU()
   - an open and readable and possibly writable file. */


boolean SetLRUWrFlag(LRUentry *refEntry);

/* SetLRUWrFlag() sets the write flag of the buffer entry, referenced by
   refEntry to TRUE. This causes the corresponding page to be written back to
   its file when necessary. See also comment concerning type LRUentry. */
