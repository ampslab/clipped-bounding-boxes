...
static void PrintEntry(RSTREE R,
                       ...
                       Rint level,
                       FILE *stream);
static Rint IndOfFaultyDataSort(RSTREE R, refnode node);
static void ChckCnst(RSTREE R,
                     Rint level,
                     boolean *consistency,
                     Rint *upperLevel,
                     typinterval *upperRect,
                     OneDKey *upperPrevKey,
                     OneDKey *upperKey,
                     Rpnint *upperPage,
                     Rint *lowerLevel,
...

{
  ...
  CopyRect((*R).numbofdim,maxBB,(*par).maxBB);
  SetCheck(R,TRUE);
  SetVersion(R);
...

{
  ...
  if (*inserted) {
    MakeDataEntry(R,rectangle,&key,info,(*R).IR_entry);
...

{
  ...
  for (d= 0; d < (*R).numbofdim; d++) {
    ...
    if (d < (*R).numbofdim - 1) {
      ...
      fprintf(stream,strans(" %16Y",s),*(refOneDKey)ptr);
      ptr+= SIZEOneDKey;
      /* ptr behind key */
      if (level == 0) {
    ...
    if ((*R1).storkind == pri) {
      ...
      if ((*R1).storkind == lru) {
        if (! LRUSatisfies((*R1).LRU,2 * (*R1).parameters._.rootlvl + 1)) {
          setRSTerr(R1,"SpJoinCount",nDirBufPagesLss);
          return FALSE;
        }
...

{
  ...
  if (twiceopen) {
    ...
    if ((*R1).storkind == pri) {
      ...
      if ((*R1).storkind == lru) {
        if (! LRUSatisfies((*R1).LRU,2 * (*R1).parameters._.rootlvl + 1)) {
          setRSTerr(R1,"SpJoin",nDirBufPagesLss);
          return FALSE;
        }
        success= OpenBufRSTIdentity(&R2,R1);
      }
...

{
  ...
  if (twiceopen) {
    ...
    if ((*R1).storkind == pri) {
      ...
      if ((*R1).storkind == lru) {
        if (! LRUSatisfies((*R1).LRU,2 * (*R1).parameters._.rootlvl + 1)) {
          setRSTerr(R1,"XSpJoinCount",nDirBufPagesLss);
          return FALSE;
        }
        success= OpenBufRSTIdentity(&R2,R1);
      }
...

{
  ...
  if (twiceopen) {
    ...
    if ((*R1).storkind == pri) {
      ...
      if ((*R1).storkind == lru) {
        if (! LRUSatisfies((*R1).LRU,2 * (*R1).parameters._.rootlvl + 1)) {
          setRSTerr(R1,"XSpJoin",nDirBufPagesLss);
          return FALSE;
        }
        success= OpenBufRSTIdentity(&R2,R1);
      }
...

{
  ...
  if (twiceopen) {
    ...
    if ((*R1).storkind == pri) {
      ...
      if ((*R1).storkind == lru) {
        if (! LRUSatisfies((*R1).LRU,2 * (*R1).parameters._.rootlvl + 1)) {
          setRSTerr(R1,"XJoinCount",nDirBufPagesLss);
          return FALSE;
        }
        success= OpenBufRSTIdentity(&R2,R1);
      }
...

{
  ...
  if (twiceopen) {
    ...
    if ((*R1).storkind == pri) {
      ...
      if ((*R1).storkind == lru) {
        if (! LRUSatisfies((*R1).LRU,2 * (*R1).parameters._.rootlvl + 1)) {
          setRSTerr(R1,"XJoin",nDirBufPagesLss);
          return FALSE;
        }
        success= OpenBufRSTIdentity(&R2,R1);
      }
...

{
  ...
  if (R == NULL) {
    setRSTerr(R,"GetMaxBB",RNULL);
    return FALSE;
  }
  
  CopyRect((*R).numbofdim,(*R).parameters._.maxBB,maxBB);
  return TRUE;
}
...

{
  ...
  *consistent= *rootMBBok && *otherMBBsKeysOk;
  return TRUE;
}
...

{
  Rint i, next;
  refnode n;
  void *ptrNi, *ptrNiKey, *ptrNiRect;
  void *ptrdatPrvKey, *ptrdatFaultKey;
  void *ptrdirPrvKey, *ptrdirActKey;
  ...
  if (level != 0) {
    ...
    do {
      ...
      if (next > 0) {
        ...
        ptrNiKey= (*R).L[next].N;
        ptrNiKey+= (*R).Pent0 + (*R).entPkey;
        key= *(refOneDKey)ptrNiKey;
        *dataFaultyInd= 0;
      }
      else {
        ...
        ptrNiKey+= (*R).Pent0 + (*R).entPkey;
        key= *(refOneDKey)ptrNiKey;
        *dataFaultyInd= IndOfFaultyDataSort(R,(*R).L[next].N);
        if (*dataFaultyInd != 0) {
          ptrdatFaultKey= (*R).L[next].N;
          ptrdatFaultKey+= (*R).Pent0 + *dataFaultyInd * (*R).DATA.entLen + (*R).entPkey;
          ptrdatPrvKey= ptrdatFaultKey - (*R).DATA.entLen;
          *dataPrevKey= *(refOneDKey)ptrdatPrvKey;
          *dataFaultyKey= *(refOneDKey)ptrdatFaultKey;
        }
      }
      ptrdirActKey= ptrNi + i * (*R).DIR.entLen + (*R).entPkey;
      ptrdirPrvKey= ptrdirActKey - (*R).DIR.entLen;
      sortOK= i == 0 || *(refOneDKey)ptrdirPrvKey <= *(refOneDKey)ptrdirActKey;
      
      ptrNiRect= ptrNi + i * (*R).DIR.entLen;
      if (*dataFaultyInd == 0 && sortOK && RectsEql((*R).numbofdim,ptrNiRect,mbb) && *(refOneDKey)ptrdirActKey == key) {
        ChckCnst(R,
                 next,
                 consistency,
                 upperLevel,
        ...
        *upperLevel= level;
        ...
        if (i > 0) {
          *upperPrevKey= *(refOneDKey)ptrdirPrvKey;
        }
        else {
          *upperPrevKey= 0;
        }
        *upperKey= *(refOneDKey)ptrdirActKey;
...

{
  ...
  if ((*node).s.nofentries > 1) {
    ptrNi+= (*R).Pent0 + (*R).entPkey;
    prevkey= *(refOneDKey)ptrNi;
    i= 1;
    do {
      ptrNi+= (*R).DATA.entLen;
      key= *(refOneDKey)ptrNi;
      if (key >= prevkey) {
        prevkey= key;
      }
      else {
...
void SetOffsets(RSTREE R,
  ...
  /* aligned lengths: */
  (*R).DIR.entLen= (*R).SIZErect + SIZEOneDKey + SIZEptrtosub;
  AlignInt(&(*R).DIR.entLen,SIZEatomkey);
  (*R).DATA.entLen= (*R).SIZErect + SIZEOneDKey + infoSize;
  AlignInt(&(*R).DATA.entLen,SIZEatomkey);

  (*R).pttsLen= (*R).DIR.entLen - ((*R).SIZErect + SIZEOneDKey);
  (*R).infoLen= (*R).DATA.entLen - ((*R).SIZErect + SIZEOneDKey);
  
  /* offsets in entries: */
  /*** (*R).entPrect= 0 ***/
  (*R).entPkey= (*R).SIZErect;
  (*R).entPptts= (*R).SIZErect + SIZEOneDKey;
  (*R).entPinfo= (*R).SIZErect + SIZEOneDKey;
