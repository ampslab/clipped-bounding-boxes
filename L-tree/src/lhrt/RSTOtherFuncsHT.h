...
/**:   1D-key R-tree
                                                                        **/
/* The implementation is reliant on an indexing function, providing
   a key for each rectangle.
   For each record to be inserted, in addition to the rectangle, it
   demands a key of type OneDKey (see RSTTypes.h), to be provided by
   an indexing function.
   See also publication:
   "Hilbert R-tree: An improved R-tree using fractals"
   [I. Kamel, C. Faloutsos; 1994].
   Functions, based on arbitrary space filling curves, may be used
   for indexing of course. */
...
boolean  CreateRST(const char   *name,
         ...
         /* CreateRST creates a tree on secondary memory.
            ...
            maxBB: (1D-key R-tree specific):
            is the bounding box of the entire data space.
            The complete data set to be inserted should reside inside maxBB.
            Violating this rule may damage the consistency of the tree. */
...
/*   Main memory residing trees are kept on two separate RAM disks; one for
     ...
     RSTree provides early detection of memory limitation, a facility,
     consisting of two parts: A guarding instance, which sets a lock when the
     risk of overflow is detected for a RAM disk, and a continuative instance,
     which attempts to extend a RAM disk when it is locked.
     The constants below control the behavior of the kit. */
...
boolean  InquireRSTDesc(t_RT     rst,
         ...
         /* InquireRSTDesc provides some useful information about the tree
            referenced by rst.
            (1D-key R-tree specific) NOTE:
            An important parameter missing here, is the bounding box of valid
            1D keys, defined during creation of the tree. Concerning
            this matter, see GetMaxBB.
...
boolean  GetMaxBB(t_RT         rst,
                  typinterval  *maxBB);
         
         /* (1D-key R-tree specific) GetMaxBB provides information about
            the area of valid 1D keys.
            
            maxBB: (1D-key R-tree specific):
            is set to the bounding box of valid 1D keys as defined
...
boolean  InsertRecord(t_RT               rst,
         ...
         /* InsertRecord inserts a new record in the tree. Its behavior
            ...
            key:
            passes the 1D key part of the new record.
            The 1D key has to be computed with respect to maxBB as
            passed during creation of the tree.
...
boolean  CheckConsistency(t_RT         rst,
         ...
         /* CheckConsistency first checks whether the stored root MBB
            ...
            whether all other MBBs and the 1D keys correctly represent
            their child pages, and whether the keys in the pages are
            ascendingly sorted.
            If it finds an incorrect MBB/key, it breaks at the first such and
            sets otherMBBsKeysOk to FALSE.
            ...
            otherMBBsKeysOk:
            contains TRUE if all MBBs/keys correctly represent their child
            pages and the keys are properly sorted, otherwise FALSE.
            ...
            storedPrevKey:
            is the previous key in the parent page.
            
            storedKey:
            is the (possibly) inconsistent key in the parent page.
            ...
            childsKey:
            is the first key of the child page.
            
            childPageNr:
            is the page number of the child page.
            
            dataFaultyInd:
            - is set to 0 if the keys of the encountered data pages were
              correctly sorted.
            - is set to the index of the first entry in the currently traced
              data page whose key is sorted reverse.
            
            dataPrevKey, dataFaultyKey:
            iff dataFaultyInd != 0, dataFaultyKey contains the first reversely
            sorted data key, dataPrevKey its predecessor; otherwise these
