...
#//
#// Implementation of the Hilbert curve coordinates: see hilbert.c
#//
...
/* includes concerning the specific 1D key: */
#include "hilbert.h"
...
#define CTREE_SUFF ".HRT.MCR"
#define FB_N_RECS 200
/* 1D key specific constants: */
#define epsFac 1.0e-15 /* extend maxBB by (width * epsFac) */
...
/* 1D key specific types: */
typedef OneDKey *refINTpoint;
...
boolean opendata(boolean UpdateOrSearch);
   void closedata(void);
...
boolean GetTestParameters(File file,
   ...
   void exeRegionCountNoDead(void);
...
boolean exeCheckConsistency(boolean verbose);
   ...
   void exeLTreeComputations(t_RT R);
...
void myPrintTimes(char* displayPrefix);
...
/* 1D key specific declarations: */
boolean PointInMaxBB(typcoord *p);
void GetAndPrintOneDKeyParams(t_RT R);
void GetCenter(const typinterval *rectangle, typcoord *center);
void point2INTpoint(const typcoord *p, refINTpoint Ip);
void SetMaxBB(void);
void RetrieveMaxBB(void);


/* global variables local to this file, standard: prefix: GV */
Rint GVnumbOfDim, GVrectSize, GVpointSize, GVinfoSize, GVINTpointSize;
...
Rlint GVnumFound;
Rlint GVpairCount;
/* global variables local to this file, 1D key specific: prefix: GV: */
int GVbitsPerDim;
long long GVsideMax;
typinterval *GVmaxBB= NULL;
typcoord *GVmaxBBdists= NULL;
...

{
  ...
  do {
    ...
    do {
      ...
      if (mainchoose == '0')
        mainchoose = 'c';
    } while ((mainchoose != 'c') && (mainchoose != 'C') &&
             (mainchoose != 'm') && (mainchoose != 'o') &&
             (mainchoose != 'O') && (mainchoose != 'l') &&
             (mainchoose != '.') && (mainchoose != 's') &&
             (mainchoose != 'R') && (mainchoose != 'K') &&
             (mainchoose != 'S') && (mainchoose != 'G') &&
             (mainchoose != 'X') && (mainchoose != 'I') &&
             (mainchoose != 'E') && (mainchoose != '`') &&
             (mainchoose != '-') && (mainchoose != 'q'));
    GVdummyCh= GetChar();
    WorkMenu= TRUE;
    if (mainchoose != 'q') {
      if (mainchoose == '-') {
      ...
      else if (mainchoose == 'X') {
        exeLTreeComputations(GVrst);
      }
      else if (mainchoose == '`') {
        exeComputePageSizes();
        WorkMenu= FALSE;
      ...
      if (WorkMenu) {
        ...
        printf("## %36s,\n","RegionQueryNoDeadSpace = \"o\"");
        printf("## %36s,\n","SpatialJoinCountNoDeadSpace = \"k\"");
        printf("## %36s,\n","CheckConsistency = \"C\"");
        printf("## %36s,\n","PathsDump = \"P\"");
        printf("## %36s,\n","ASCIIdump = \"A\"");
        printf("## %36s,\n","DirLevelDump = \"L\"");
        ...
        do {
          ...
          if ( choose == '0')
            choose = 'i';
        } while ( (choose != 'i') && (choose != 'd') && (choose != 'x') &&
                  (choose != 'e') && (choose != 'E') && (choose != 'r') &&
                  (choose != '[') && (choose != 'j') && (choose != '=') &&
                  (choose != 'J') && (choose != '#') && (choose != 'N') &&
                  (choose != 'D') && (choose != 'C') && (choose != 'P') &&
                  (choose != 'A') && (choose != 'L') && (choose != 'M') &&
                  (choose != '-') && (choose != 'o') && (choose != 'k'));
        GVdummyCh= GetChar();
        if (choose == '-') {
        }
        else if (choose == 'i') {
          ...
          exeMediaReorg();
           ...
           printf("Executing count range query taking empty space into account\n");
           QueryName();
           if (openquery()) {
             exeRegionCountNoDead();
             closequery();
           }
        }
        else if (choose == 'k') {
          printf("Executing count spatial join taking empty space into account\n");
          exeSpatialJoinCountNoDead();
        }
      }
    }
    else {
...
{
  printf("%s%s%s\n",
"\n##   --- C-HRTree ---  ","important notes",":"
  );
  printf("%s\n",
"## - C-tree based variant of Hilbert R-tree (lhrt). The below functionality" );
  printf("%s\n",
...

{
  ...
  scanf("%s",GVdistName);
  GVdummyCh= GetChar();
  if (strlen(GVdistName) == 1 && GVdistName[0] == '0')
    strlcpy(GVdistName, "test_data/test_data\0", 20);
...
void GetAndPrintParamsForUsage(t_RT R) {
  ...
  GVINTpointSize= numbOfDimensions * sizeof(OneDKey);
  /* print parameters: */
...

{
  ...
  if (strlen(GVquerName) == 1 && GVquerName[0] == '0')
    strlcpy(GVquerName, "test_data/test_data_queries\0", 28);
}
...

{
  ...
  printf("%s\n",GVdistName);
  if (! OpenRdOnlyF(GVdistName,&GVdataFile)) {
...

{
  ...
  printf("%s\n",GVquerName);
  if (! OpenRdOnlyF(GVquerName,&GVquerFile)) {
...

{
  ...
  scanf("%d",&dirpagelen);
  if (dirpagelen == 0)
    dirpagelen = 256; // used for testing with test_data to create 2-level tree
  printf("     Size of a data page: ");
  scanf("%d",&datapagelen);
  if (datapagelen == 0)
    datapagelen = 256; // used for testing with test_data to create 2-level tree
  printf("    Number of dimensions: ");
  scanf("%d",&numbOfDimensions);
  if (numbOfDimensions == 0)
    numbOfDimensions = 2;
  ...
  SetMaxBB();
  printf("%s%s,%d,%d,%d,%d,%d,%s%s\n","CreateRST(",GV_RfileName,dirpagelen,datapagelen,GVnumbOfDim,GVinfoSize,unique,"MaxBB","):");
  if (CreateRST(GV_RfileName,dirpagelen,datapagelen,GVnumbOfDim,GVinfoSize,unique,GVmaxBB)) {
    printf("Done\n");
  }
  else {
...

{
  ...
  printf(strans("%s%d,%d,%d,%d,%d,%s,%P,%P,%d%s\n",s),"CreateMainMemRST(",dirpagelen,datapagelen,GVnumbOfDim,GVinfoSize,unique,"MaxBB",dirramsize,dataramsize,TRUE,"):");
  if (CreateMainMemRST(&GVrst,dirpagelen,datapagelen,GVnumbOfDim,GVinfoSize,unique,GVmaxBB,dirramsize,dataramsize,TRUE)) {
    printf("Done\n");
    printf("\n");
...

{
  ...
  if (GVend == 0) {
    GVend= LenOfF(file) / GVrectSize;
  }
  if (! SetPosF(file,GVbegin * GVrectSize)) {
    DisplErr(name,GVbegin);
    return FALSE;
  }
...

{
  ...
  t_FB fb;
  typcoord *center= allocM(GVpointSize);
  refINTpoint INTcenter= allocM(GVINTpointSize);
  OneDKey key;
  
  CountsOn0(GVrst);
  CreateFileBuf(&fb,GVrectSize,FB_N_RECS,GVdataFile);
  
  i= GVbegin;
  StartTimers();
  do {
    if (! RdBufBytes(fb,rectangle)) {
      DisplErr(GVdistName,i);
      freeM(&info); freeM(&infoStored); freeM(&rectangle);
      freeM(&center); freeM(&INTcenter);
      DisposeFileBuf(&fb);
      return;
    }
    /*** Begin NECESSARY 1D key specific operations ***/
    GetCenter(rectangle,center);
    if (! PointInMaxBB(center)) {
      printf("FAILURE: Calculated center point outside MaxBB:\n");
      printf("center point:\n");
      PrintPoint(center);
      printf("MaxBB:\n");
      PrintRect(GVmaxBB);
    ...
    point2INTpoint(center,INTcenter);
    key= hilbert_c2i(GVnumbOfDim,GVbitsPerDim,INTcenter);
    /*** End   NECESSARY 1D key specific operations ***/
    i++;
    ...
    success= InsertRecord(GVrst,rectangle,key,info,&inserted,infoStored);
    if (! success) {
...

{
  ...
  CountsOn0(GVrst);
  GVCompCount0();
  CreateFileBuf(&fb,GVrectSize,FB_N_RECS,GVquerFile);
...
#ifdef NDEBUG
  int status = system("sync");
  status = system("sudo bash -c \"echo 3 > /proc/sys/vm/drop_caches\"");
//  status = system("dd if=/dev/zero of=deleteME bs=1M count=128 2> /dev/null");
  status = system("sync");
//  status = system("cat deleteME > /dev/null");
  if (status != 0)
    printf( "Warning: unable to drop caches (status: %d)!\n", status );
  printf( "\n[OS caches have been dropped!]\n" );
#endif
  ...
  do {
    ...
    success= RegionCountNoDead(GVrst,
                         rects,i,qPtr,DirIntersects,DataIntersects,
                         &numfound);
    i++;
    if (! success) {
      printf("FAILURE(RegionCount)\n");
...
void exeSpatialJoinCountNoDead(void)

{
  ...
  */
  if (GVrst2 != GVrst) {
    printf( "Adding L-Tree improvements to the secondary tree. Improvements for the main tree have to be added manually prior to executing the join! \n");
    exeLTreeComputations(GVrst2);
  }
...
//  status = system("cat deleteME > /dev/null");
  ...
  success= SpJoinCountNoDead(GVrst,GVrst2,&paircount);
  if (! success) {
    printf("FAILURE(SpJoinCountNoDead)\n");
...

{
  ...
  Rint rootLevel;
  int d, lv;
  Rint upperLevel, lowerLevel, dataFaultyInd;
  Rpnint upperPage, lowerPage;
  boolean success, consistent, rootMBBok, otherMBBsKeysOk;
  ...
  OneDKey upperPrevKey, upperKey, lowerKey, dataPrevKey, dataFaultyKey;
  Rint path[50];
  Rpnint nodecount[50];
  ...
  if (verbose) {
    printf("height: %d\n",rootLevel+1);
    printf("CHECKING: Consistency of all MBBs and keys in the tree\n");
  }
  ...
  if (consistent) {
    if (verbose) {
      printf("\n --- all MBBs and keys are consistent ---\n\n");
    ...
    if (otherMBBsKeysOk) {
      printf("\n --- the other MBBs and keys are consistent ---\n\n");
    }
    else {
      printf("\n ##### INCONSISTENT MBBs / keys resp. #####\n\n");
      ...
      printf("Registered rectangle:\n");
      for (d= 0; d < GVnumbOfDim; d++) {
        printf("%24.16e%24.16e\n",upperRect[d].l,upperRect[d].h);
      }
      printf(strans("    Previous registered key:  %Y\n",s),upperPrevKey);
      printf(strans("             Registered key:  %Y\n",s),upperKey);
      printf(strans("                    in page:  %N\n",s),upperPage);
      printf("Actual enclosing rectangle of the child-node:\n");
      for (d= 0; d < GVnumbOfDim; d++) {
        printf("%24.16e%24.16e\n",lowerRect[d].l,lowerRect[d].h);
      }
      printf(strans("First key of the child-node:  %Y\n",s),lowerKey);
      if (dataFaultyInd != 0) {
        printf(strans("      bad sort in data page:  %N\n",s),lowerPage);
        printf(strans("              faulty, index:  %d\n",s),dataFaultyInd);
        printf(strans("          previous data key:  %Y\n",s),dataPrevKey);
        printf(strans("            faulty data key:  %Y\n",s),dataFaultyKey);
      }
      else {
...
void exeLTreeComputations(t_RT R) {
  ...
  strlcpy(name,GVdistName,sizeof(name));
  strlcat(name,CTREE_SUFF,sizeof(name));

  printf("Maximum number of deadly points per node (0 -> 1 per corner): ");
  scanf("%d",&max_dpoints);
  const Rint num_corners = pow( 2, GVnumbOfDim );
  if (max_dpoints == 0 ) max_dpoints = num_corners;

  printf("Dead-space computation method (Skyline -> 0; Staircase -> 1): ");
  scanf( "%d", &method );

  printf("Dump to file \"%s\" or to stdout or off (f/-/o)? ", name);
  do {
    ch= GetChar();
    if ( ch == '0' )
      ch = '-';
  ...
  if (ch == 'f') {
    printf("Writing %s\n",name);
    stream= fopen(name,"w");
    success= LTreeComp( R, stream, max_dpoints, method );
    if (success) {
      fclose(stream);
      printf("Done\n");
    }
  } else {
    success= LTreeComp( R, ch == '-' ? stdout : NULL, max_dpoints, method );
  }
  if (! success) {printf("FAILURE(LTreeComp)\n"); return;}
}
...
#define DATA_RAM_SIZE (3 * DATA_P_SIZE)	/* at least 3 */
  ...
  do {
    ...
    scanf("%d",&infoLen);
    GVmaxBB= reallocM(GVmaxBB,numbDim * sizeof(typinterval));
    ...
    printf("-------------------------------------------------------------\n");
    printf("Given these parameters, you can first enter a number of\n");
    printf("entries, and then enter a page size in bytes.\n");
    printf("A too small page size provided, the minimum will be applied.\n");
    printf("-------------------------------------------------------------\n");
...
/***********************************************************************/
/***********************************************************************/
/* 1D-key R-tree specific functions */

boolean PointInMaxBB(typcoord *p)

{
  int d;
  typinterval *maxBB= GVmaxBB;
  
  d= 0;
  do {
    if (*p < (*maxBB).l || *p > (*maxBB).h) {
      return FALSE;
    }
    p++; maxBB++;
    d++;
...
{
  GVbitsPerDim= (8 * sizeof(OneDKey)) / GVnumbOfDim;
  GVsideMax= (long long)2 << GVbitsPerDim - 1;
                         /* signed constant preserves overflow detection */
  printf("OneDKey resolution:\n");
  printf("%2d bits overall,\n",8 * (int)sizeof(OneDKey));
  printf("%2d bits per dimension,\n",(int)GVbitsPerDim);
  printf(strans("%2L grid positions per dimension.\n",s),GVsideMax);
}
...

{
  ...
  for (d= 0; d < GVnumbOfDim; d++) {
    center[d]= (rectangle[d].l + rectangle[d].h) * 0.5;
  }
}
...

{
  int i, d, numbrecs;
  Rpint lof, ckprod;
  typinterval *datarect= allocM(GVrectSize);
  double dist, epsilon;
  t_FB fb;
  
  GVmaxBB= reallocM(GVmaxBB,GVrectSize);
  GVmaxBBdists= reallocM(GVmaxBBdists,GVpointSize);
  
  opendata(FALSE);
  printf("Computing maximum bounding box:\n");
  lof= LenOfF(GVdataFile);
  numbrecs= lof / GVrectSize;
  ckprod= numbrecs * GVrectSize;
  if (ckprod != lof) {
    printf(strans("WARNING: File length: %P %% %d :sizeof(typrect) != 0\n",s),lof,GVrectSize);
  }
  printf("Number of records: %d\n",numbrecs);
  if (numbrecs == 0) {
    printf("ERROR: Maximum bounding box unset!\n");
    freeM(&datarect);
    closedata();
    return;
  }
    ...
    for (i= 1; i < numbrecs; i++) {
      if (! RdBufBytes(fb,datarect)) {
        DisplErr(GVdistName,i);
        printf("ERROR: Maximum bounding box insufficiently set!\n");
        freeM(&datarect);
        DisposeFileBuf(&fb);
        closedata();
        return;
      ...
      for (d= 0; d < GVnumbOfDim; d++) {
        if (datarect[d].l < GVmaxBB[d].l) {
          GVmaxBB[d].l= datarect[d].l;
        }
        if (datarect[d].h > GVmaxBB[d].h) {
          GVmaxBB[d].h= datarect[d].h;
        }
      }
    }
  }
  for (d= 0; d < GVnumbOfDim; d++) {
    dist= GVmaxBB[d].h - GVmaxBB[d].l;
    epsilon= epsFac * dist;
    GVmaxBB[d].l-= epsilon;
    GVmaxBB[d].h+= epsilon;
    GVmaxBBdists[d]= GVmaxBB[d].h - GVmaxBB[d].l;
  }
  freeM(&datarect);
  DisposeFileBuf(&fb);
  closedata();
...

{
  ...
  for (d= 0; d < GVnumbOfDim; d++) {
    Ip[d]= (p[d] - GVmaxBB[d].l) / GVmaxBBdists[d] * GVsideMax;
  }
}
...

{
  Rlint dirDemands, dataDemands, dirGets, dataGets, dirReads, dataReads,
        dataOptimal, Demands, Gets, Reads;
  boolean success;
  
  success= GetCountRead(R,
                        &dirDemands,&dataDemands,
                        &dirGets,&dataGets,
                        &dirReads,&dataReads, &dataOptimal);
  ...
  printf(strans("%s%L\n",s),"OPTIM dataDemands: ",dataOptimal);
...
void PrintOptimals(t_RT R) {
  ...
  success= GetOptimalCounts( R, &max_hurt );

  printf(strans("%s%L\n",s),"MOST dataDemands: ",max_hurt);
}
...
{
  Rlint MdirDemands, MdataDemands, MdirGets, MdataGets, MdirReads, MdataReads, MdataOptimal;
  Rlint SdirDemands, SdataDemands, SdirGets, SdataGets, SdirReads, SdataReads, SdataOptimal;
  Rlint dirDemands, dataDemands, dirGets, dataGets, dirReads, dataReads,
        dataOptimal, Demands, Gets, Reads;
  boolean success;
  ...
  success= GetCountRead(Rmain,
                        ...
                        &MdirGets,&MdataGets,
                        &MdirReads,&MdataReads, &MdataOptimal);
  success= GetCountRead(Rsec,
                        &SdirDemands,&SdataDemands,
                        &SdirGets,&SdataGets,
                        &SdirReads,&SdataReads, &SdataOptimal);
  ...
  dataOptimal = MdataOptimal + SdataOptimal;
  Demands= dirDemands+dataDemands;
  Gets= dirGets+dataGets;
  Reads= dirReads+dataReads;
  /* if LRU Buffered, set dir- and data-specific R/W counts to 0 (though
...
{
  Rlint dirDemands, dataDemands, dirGets, dataGets, dirReads, dataReads,
        dataOptimal, dirPuts, dataPuts, dirWrites, dataWrites,
        dirIOs, dataIOs, IOs,
...

{
  ...
  typinterval *maxbb= allocM(GVrectSize);
  
  if (GetMaxBB(R,maxbb)) {
    printf("Maximum bounding box:\n");
    for (d= 0; d < GVnumbOfDim; d++) {
      printf("dimension %d:  % .2f  ..  % .2f\n",d,maxbb[d].l,maxbb[d].h);
    }
  }
