...
{
  printf("%s%s%s\n",
"\n##   --- TestLRRSTree ---  ","important notes",":"
  );
  printf("%s\n",
"## - L-tree based variant of RR*-tree (rrst). The below functionality is" );
  printf("%s\n",
...
void exeLTreeComputations(t_RT R) {
  ...
  strlcpy(name,GVdistName,sizeof(name)); //TODO: pass the name as argument, to handle the secondary tree
...
#define DATA_RAM_SIZE (3 * DATA_P_SIZE)	/* at least 3 */
  ...
  do {
    ...
    printf("Attempting to temporary create a corresponding R-tree. Result:\n");
    success= CreateMainMemRST(&R,
                              dirByteQty,
                              dataByteQty,
                              numbDim,
